...
| 6 | **Data model and migrations** — legacy field shapes, `ensureArray`, Firebase turning sparse arrays into objects and dropping empty ones, uid vs username keying, and anything still reading a shape nothing writes. | DONE 2026-08-24 | 4 real. Ancestor service requests were written and never read — no inbox existed at all, and requests predating the move out of `church/data` were stranded; inbox built, legacy ones folded in. Firebase cannot store an empty array, so deleting the last service, prayer form or Johrei time brought the defaults back on next open; deliberately-emptied lists are now recorded. Settings loaded the profile by username, which is derived from the email prefix and can collide — it showed another member's phone and home address. The retired `classes` preference is now cleaned out and ignored. |
| 7 | **Client UI correctness** — rendering, admin-only gating (does a member ever see an admin control?), modal lifecycle, duplicate event listeners across sign-in cycles, and the back/tab navigation. | DONE 2026-08-24 | 2 real. Revealing an admin element also forced `display: inline-flex` on it, so the full-width YouTube and Member Accounts panels became shrink-to-fit boxes rendering on top of themselves and overflowing the screen — visible in the admin's own screenshot. Hiding no longer sets a display. Closing a modal only hid it: a slow inbox read could then write its contents into whatever modal was opened next, putting a member's prayer text inside the service-times form and destroying it; modals now carry a token and are emptied on close. Gating itself is sound — a member sees no admin control anywhere. Tabs, the More back bar and listener rewiring across sign-in cycles all verified clean. |
| 8 | **Privacy and security** — what each role can actually read, PII in notification payloads and in the public Actions logs, the public repo, and anything a member could reach that they should not. | DONE 2026-08-24 | 3 real, all the same shape: the interface filtered, the database did not. Every signed-in account could read every member's profile — name, email, phone, home address; now own-profile only. Ancestor enrolments (family names, dates of passing, the member's contact details) sat in the shared node behind a UI filter; moved to a per-member node the rules enforce. The YouTube API key was a credential in member-readable data; moved to an admin-only node, and only the admin's browser calls YouTube now. Push payloads and the public Actions log were re-checked and are clean. Outstanding, needs the church's decision: signup is open to anyone on the internet. |
